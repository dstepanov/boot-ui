...
/**
 * Proves the Resilience panel is genuinely live on the reactive stack: the shared read path finds the
 ...
 * Resilience backend's optional-dependency guards the servlet sample cannot: BootUI must start and
...
class WebFluxResilienceIntegrationTest {
    ...
    void reactivePolicyInventoryAndCapturedEventsAreReported() throws Exception {
        ...
        HttpResponse<String> beforeAnyCall = get(client, "/bootui/api/resilience");
        assertThat(beforeAnyCall.statusCode()).isEqualTo(200);
        assertThat(beforeAnyCall.body())
                .contains("\"resiliencePresent\":true")
        ...
        HttpResponse<String> exercised = get(client, "/api/sample/resilience");
        assertThat(exercised.statusCode()).isEqualTo(200);

        HttpResponse<String> report = get(client, "/bootui/api/resilience");
