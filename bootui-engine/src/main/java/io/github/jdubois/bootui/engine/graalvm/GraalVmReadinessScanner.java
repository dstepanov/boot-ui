 ...
 */
public final class GraalVmReadinessScanner {
     ...
     * the supplied base packages, and surveys dependencies per the supplied settings. Base packages are read
     * <em>live</em> on every scan (the supplier is typically backed by a {@code BasePackageProvider} SPI); the
     * ArchUnit import and any dependency-repository lookups run only on demand (POST /scan), never at construction.
     * The adapter supplies the {@link ReachabilityMetadataRepository} that performs reachability-metadata
     * lookups, keeping the engine free of any JSON/HTTP library.
     */
    public static GraalVmReadinessScanner usingClasspath(
            Supplier<List<String>> basePackagesSupplier,
            GraalVmDependencySettings dependencySettings,
        ...
        return new GraalVmReadinessScanner(
                ...
                new GraalVmDependencyScanner(
                        repository,
                        dependencySettings.repositoryLookupEnabled(),
                        dependencySettings.maxRepositoryLookups()),
                clock);
    }

    public GraalVmScanResult initialResult() {
    ...
    public void cancelDependencyScan() {
    ...
    public record GraalVmScanResult(GraalVmReadinessReport report, GraalVmMetadata metadata) {}
