...
/**
 * Framework-neutral, independently bounded buffer of recently captured resilience events.
 *
 ...
 * {@code CircuitBreakerMaintenance} state-change callback. The recorder itself imports no resilience
 ...
 * sources, so resilience traffic can never evict unrelated entries.</p>
 *
 ...
 */
public final class ResilienceEventRecorder {
    ...
    /** One captured event, before it is flattened to the stable {@link ResilienceEventDto} contract. */
    ...
    private volatile TraceIdProvider traceIdProvider = ResilienceEventRecorder::mdcTraceId;

    public ResilienceEventRecorder(boolean enabled, int maxEntries) {
    ...
    public void setTraceIdProvider(TraceIdProvider traceIdProvider) {
        this.traceIdProvider = traceIdProvider == null ? ResilienceEventRecorder::mdcTraceId : traceIdProvider;
    }

    /**
     * Captures one resilience event. A {@code null}/blank policy name or outcome is rejected rather than
     ...
     */
    public List<ResilienceEventDto> recentDtos(int limit) {
        List<CapturedEvent> events = recent();
        int cap = limit <= 0 ? events.size() : Math.min(limit, events.size());
        List<ResilienceEventDto> dtos = new ArrayList<>(cap);
        for (int i = 0; i < cap; i++) {
    ...
    static ResilienceEventDto toDto(CapturedEvent event) {
        return new ResilienceEventDto(
                "resilience-" + event.id(),
                event.timestamp(),
                event.policyName(),
                event.policyType(),
