...
async function fetchReport() {
  error.value = null
  try {
    report.value = await getJson('api/resilience')
    lastFetched.value = Date.now()
  } catch (e) {
    error.value = describeLoadError(e, 'Unable to load resilience policies')
  }
}
...
const available = computed(() => manifestAvailable.value && report.value?.resiliencePresent === true)

const unavailableReason = computed(() => {
  if (!manifestAvailable.value) return manifestUnavailableReason.value
  return report.value?.unavailableReason || 'No supported resilience library is present.'
})
  ...
  <div>
    ...
    <template v-else-if="!manifestAvailable || report">
      <div v-if="!available" class="alert alert-warning">
        <strong>Resilience is unavailable.</strong>
      ...
      <template v-else>
        ...
        <div v-if="!report.captureEnabled" class="alert alert-secondary small py-2">
          Event capture is currently disabled (<code>bootui.resilience.enabled=false</code>). Policies below are still
          read live; no retry, rejection, timeout or breaker-transition events are being recorded.
        </div>
        ...
        <div v-if="report.totalPolicies === 0" class="alert alert-secondary">
          No resilience policies declared yet. Register a circuit breaker, retry, rate limiter, bulkhead or timeout and
          refresh this panel.
        </div>
        ...
        <template v-else>
          <div class="mb-3 d-flex gap-2 flex-wrap">
            <input
              v-model="filter"
              class="form-control form-control-sm resilience-filter-input"
              aria-label="Filter resilience policies"
              placeholder="Filter by policy name, protected operation, or provider…"
            />
            <select
              v-model="typeFilter"
              class="form-select form-select-sm resilience-type-select"
              aria-label="Filter resilience policies by type"
            >
          ...
          <div class="table-responsive mb-4">
            <table class="table table-sm table-hover align-middle resilience-policy-table">
              <thead>
                <tr>
                  <th>Policy</th>
...
<style scoped>
.resilience-filter-input {
  max-width: 26rem;
}

.resilience-type-select {
  max-width: 12rem;
}
