...
import FaultTolerance from './FaultTolerance.vue'
...
describe('FaultTolerance', () => {
  ...
  it('renders the policy inventory, live breaker state and captured events', async () => {
    ...
    wrapper = mount(FaultTolerance)
    await flushPromises()

    const text = wrapper.text()
    expect(text).toContain('Fault Tolerance')
    expect(text).toContain('Inspect the retries, circuit breakers, rate limits, bulkheads, timeouts, and fallbacks')
  ...
  it('explains that the panel is unavailable when no fault tolerance library is present', async () => {
    vi.stubGlobal(
      'fetch',
      vi.fn(() =>
        Promise.resolve(
          jsonResponse({
            faultTolerancePresent: false,
            unavailableReason: 'No supported fault tolerance library is present',
    ...
    expect(wrapper.text()).toContain('Fault tolerance data is unavailable.')
    expect(wrapper.text()).toContain('No supported fault tolerance library is present')
  })
  ...
  it('still shows captured events when no policy is declared', async () => {
    ...
    const text = wrapper.text()
    expect(text).toContain('No fault tolerance policies declared yet.')
    expect(text).toContain('inventoryRetry')
    expect(wrapper.find('.fault-tolerance-event-table').exists()).toBe(true)
  })
  ...
  it('prefills the filter box from the ?q= deep-link query parameter', async () => {
    ...
    expect(wrapper.get('input.fault-tolerance-filter-input').element.value).toBe('paymentGateway')
    expect(wrapper.text()).toContain('paymentGateway')
  ...
  it('filters the policy table by type', async () => {
    ...
    await wrapper.get('select.fault-tolerance-type-select').setValue('RETRY')
