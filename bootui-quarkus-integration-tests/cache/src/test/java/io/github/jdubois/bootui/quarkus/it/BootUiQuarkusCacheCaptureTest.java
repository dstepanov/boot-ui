 ...
 * {@code GET /bootui/api/cache} with a working {@code POST /clear} action — all in-process, no Docker.
 *
 ...
 * keeps the {@code cache} panel unavailable, with no {@code CacheProvider} bean). Nothing here triggers
...
class BootUiQuarkusCacheCaptureTest {
    ...
    void cachePanelReportsCachesMetricsAndClears() {
        ...
        JsonNode afterClear = findCache(probe().get("/bootui/api/cache").json(), "orders");
