...
test.describe('Resilience view', () => {
  ...
  test('filters the policy inventory by name and by type', async ({openView, page}) => {
    ...
    const filter = page.locator('input.resilience-filter-input')
    ...
    await filter.fill('')
    await page.locator('select.resilience-type-select').selectOption('BULKHEAD')
    await expect(rows).toHaveCount(1)
  ...
  test('surfaces resilience events in Live Activity with a deep link back to this panel', async ({page, request}) => {
    const triggered = await request.get('/api/sample/resilience')
    expect(triggered.ok()).toBeTruthy()
    ...
    await expect(table).toContainText('RESILIENCE', {timeout: 15_000})
